# Wine 🍷
**와인 리뷰 플랫폼 - 와인** 은 와인을 등록하고, 코드잇 부트캠프 와인의 별점을 매겨 사용자들 세계 각국의 다양한 와인을 등록하고 소개하여, 사용자들이 와인을 쉽게 접할 수 있도록 돕는 것을 목표로 합니다.

![image](https://github.com/user-attachments/assets/bb8345c2-b26d-4c34-a1e3-9daafcfbfc60)

<br>

## 1) 프로젝트 개요


### 🍷 왜 와인 프로젝트를 선정했나요?

와인 프로젝트는 리뷰 작성, 별점 평가, 다양한 모달 같은 여러 컴포넌트를 직접 구현해볼 수 있는 좋은 기회를 제공합니다. 뿐만 아니라, 단순한 로그인 기능을 넘어 구글과 카카오 간편 로그인 기능도 손쉽게 추가해볼 수 있어서, 실제 서비스에 필요한 기술을 배우고 활용할 수 있습니다. 사이트 디자인도 예쁘고 고급스러운 느낌을 주어, 사용자 경험을 한층 더 개선할 수 있다는 점이 이 프로젝트를 선택한 중요한 이유 중 하나였습니다.

<br>

### 🍷 프로젝트 내용은 뭔가요?

**랜딩페이지**
<br>**회원가입**
<br>**로그인**
<br>**와인목록** - 매달 새롭게 만나는 와인 추천 콘텐츠
<br>**와인디테일** - 직관적인 리뷰 시스템
<br>**마이프로필** - 다양한 필터로 찾는 내 맞춤 와인

<br>

### 🍷 개발환경
- Front : TypeScript, Next.js, SCSS, Axios, next.auth
- Back : [Swagger](https://winereview-api.vercel.app/docs/#/)
- 버전 및 이슈관리 : Github, Github Project
- 협업 툴 : Discord, Notion, Github, Git
- 디자인 : [Figma 디자인](https://www.figma.com/design/KKEgdM0NWLI4kSqEpdR3RB/WHYNE%5BBBB%5D?node-id=6-1760&t=WYBbUP8ZghAQ6fo6-0)

<br>

### 🍷 어떤 기대 효과가 있을까요?

#### 소비자 경험 향상 및 개인화된 와인 추천
사용자들에게 리뷰 작성과 별점 평가 기능을 통해 개인화된 와인 추천을 제공합니다. 이를 통해 소비자는 자신에게 맞는 와인을 쉽게 찾을 수 있으며, 추천 시스템을 통해 고평가된 와인을 추천받을 수 있습니다.

#### 커뮤니티 기반의 사용자 충성도 증대
사용자들이 직접 리뷰를 작성하고, 와인을 등록하며, 다른 사용자와 소통함으로써 플랫폼에 대한 충성도가 높아지고, 반복 방문이 유도됩니다. 이는 플랫폼의 지속적인 성장과 사용자 수 확대에 기여합니다.

#### 데이터 기반의 마케팅 전략 수립
수집된 리뷰와 별점 데이터는 와인 판매자와 마케터들에게 중요한 자산이 됩니다. 이를 바탕으로 타겟 마케팅, 신제품 개발, 재고 관리 등 데이터 기반의 마케팅 전략을 수립할 수 있습니다.

<br>

## 2) 프로젝트 팀 구성 및 역할

🤷 `김광호`<br>
- 페이지
  - 프로필 페이지
- 컴포넌트
  - 모달, 슬라이더 컴포넌트
<br><br>

🤷‍♂️ `김동규`<br>
- 페이지
  - 와인 목록 페이지
- 컴포넌트
  - 카드 컴포넌트, 인풋컴포넌트
<br><br>

🤷‍♀️ `오성준`<br>
- 페이지
  - 회원가입 페이지
- 컴포넌트
  - 모달, 소셜 로그인 컴포넌트, 검색 컴포넌트
<br><br>

🤷‍♂️ `윤수현`<br>
- 페이지
  - 와인 상세 페이지
- 컴포넌트
  - 카드 컴포넌트, 드랍다운 컴포넌트
<br><br>

🤷‍♀️ `정한주`<br>
- 페이지
  - 로그인 페이지
- 컴포넌트
  - GNV 컴포넌트, 버튼 컴포넌트
<br><br>

👨‍🏫 `멘토님`<br><br>
매주 2회씩 멘토링하면서 코드 피드백
    
 <br>
    
## 3) 프로젝트 수행 절차 및 방법
<img src="https://github.com/user-attachments/assets/c656201d-0014-4cb7-b7cf-4c5d65b86a08" width=550px>

<br><br>
    
## 4) 프로젝트 수행 경과

시연영상

<br>

## 5) 자체 평가 의견


### 🍷 프로젝트를 마치며
와인이라는 특정 주제를 다루며, 다양한 기능을 통해 사용자에게 가치를 제공하는 방법을 깊이 경험할 수 있었습니다. 특히, 구글과 카카오 간편 로그인 기능 같은 고난이도 작업을 진행하면서 어려움도 많았지만, 그 과정에서 크게 성장할 수 있었습니다. 또한, 컴포넌트 사용의 장점을 직접 체감하며, 효율적인 개발의 중요성을 깨달았습니다. 이번 프로젝트에서 얻은 피드백을 바탕으로, 앞으로 더 나은 프로젝트를 개발하기 위해 지속적으로 보완해 나갈 계획입니다.
