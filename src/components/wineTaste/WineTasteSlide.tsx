...
export function WineTasteSlide({ tasteValue, SlideMode, onSlideChange }: WineTasteSlideProps) {
  ...
  useEffect(() => {
    if (tasteValue) {
      setLocalSlideValues(tasteValue);
    }
  }, [tasteValue]);
    ...
    <div className="taste-slide-layer">
      {Object.entries(wineTaste).map(([tasteKey, tasteOptions], index) => (
        <SlideComponent
          key={tasteKey}
          mode={SlideMode}
          SlideOptionTitle={tasteOptions.title}
          LeftOption={tasteOptions.leftOption}
          RightOption={tasteOptions.rightOption}
          value={localSlideValues[index]}
          onValueChange={(newValue: number) => handleSliderChange(index, newValue)}
        />
      ))}
    </div>
  );
